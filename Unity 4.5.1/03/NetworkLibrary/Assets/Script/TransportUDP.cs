...
/// <summary>
/// 통신 스레드에서 실행할 처리와 스레드를 시작하는 방법을 살펴보겠습니다. 통신 스레드는
/// 메인 스레드로부터 송신 데이터가 등록되면 Socket 클래스의 Send 함수로 데이터를 송신합니다. 그다음
/// Receive 함수를 사용하여 수신 데이터를 Socket 클래스의 수신 버퍼에서 메인 스레드와 주고 받는 큐로 옮깁니다.
/// 이 과정을 스레드가 종료할 때까지 반복합니다. 이 루프를 연속으로 실행하면 다른 스레드로 처리 제어가 돌아가지 않으므로
/// 일정 간격으로 스레드를 실행합니다. 예제 프로그램에서는 5ms마다 송수신하겠습니다.
/// </summary>
